                    ...
                    <c:choose>
						...
						<c:otherwise>
							<c:forEach var="resultList" items="${resultList}">
								<tr>
									<td>${resultList.manager_id }</td>
									<td>${resultList.mg_name }</td>
									<td>
										<input type="button" value="삭제" onclick="deleteIt('${resultList.manager_id }')">
									</td>
								</tr>
							</c:forEach>
						</c:otherwise>
...
<script>
	...
	function deleteIt(id){
		if(confirm('관리자 아이디 : '+id+"\n삭제하시겠습니까?")){
			location.href = "${pageContext.request.contextPath }/adminDeleteOk.admin?manager_id="+id;
		} else{
			return false;
		}
	}
